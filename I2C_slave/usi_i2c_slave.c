...
// NOTE - this is not the slave address, this is an internal address that can be referenced by the master
// 		  to get the value of a specific variable.
//		  For our purposes, we only need one number (one sensor) but this could be expanded later
...
// These are states that we keep track of
enum
{
...
/////////////////////////////////////////////////////////////////////////////////
// Update register pointers to point to new addresses...
////////////////////////////////////////////////////////////////////////////////
void USI_update_register_pointers(char* arr[]){
	
	int i;
	for(i=0;i<USI_SLAVE_REGISTER_COUNT;i++){
		USI_Slave_register_buffer[i] = arr[i];
	}
...
ISR(USI_OVF_vect)
{
	...
	switch (USI_I2C_Slave_State)
	{
		...
		/////////////////////////////////////////////////////////////////////////
		case USI_SLAVE_RECV_DATA_ACK_SEND:
			// send ACK before because of slow clock speed			USI_SET_SDA_OUTPUT();
			USISR = USI_SLAVE_COUNT_ACK_USISR;
			
			USI_I2C_Slave_State = USI_SLAVE_RECV_DATA_WAIT;
			// internal address is set to incoming data
